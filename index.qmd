...
subtitle: "Coupling of severe climate change and extinction drive biogeographic homogenization in deep time"
date: last-modified
...
This Quarto report accompanies the manuscript *Coupling of severe
climate change and extinction drive biogoegraphic homogenization in deep
time* submitted to *Proceedings B*.

It contains the full R code and analyses used to generate the results in
the paper.
...
rpkg <- c("dplyr ggplot2 readr boot divvy terra divDyn magick 
          conflicted piggyback CoordinateCleaner fossilbrush 
           rgplates icosa tidyr tibble readr purrr patchwork
           downloadthis ggpubr stringr ggrepel")
...
conflicted::conflict_prefer(name = "intersect",winner = "dplyr", losers = c("stats", "base", "terra","lubridate"))
...
The following code is used to calculate the similarity values for each
geologic age throughout the entire Phanerozoic for the occurrences of
marine invertebrates, to examine which ages exhibit statistically
meaningful changes in the biotic composition of global communities.

## Paleobiology Database

The fossil occurrence data analysed in this study was retrieved from the
[Paleobiology Database](https://paleobiodb.org/#/) on November of 2024.
Data pre-processing made use of functions from the `fossilbrush` and
`CoordinateCleaner` R packages.

::: callout-warning
## **Before you proceed**

The following code chunk shows you how we processed the full Phanerozoic
`Pbdb` dataset that was downloaded using the `Pbdb_download_new.R`
script, which is available in our repository.

The pbdb file is too large to upload to Github, so please run the
Pbdb_download_new.R script first to generate it and store the resulting
pbdb file (here, pbdb.data.Nov2024.csv) in the same working directory as
the one you use for this script.
:::
...
Next, visualize all occurrences for each stage, using the package
`rgplates` and `icosa` on R. Please note that this version requires that
you have the [GPlates](https://www.earthbyte.org/) software (v.2.5.0 as
of writing this script) installed in your computer, as it is the most
optimal version of `rgplates`.
...
Now to visually display all the cells for which there are occurrences
for each age. To make it simple, let's turn it into a gif.
...
Next, we investigate the data by dividing it by stage and taxonomic
class. We determine the number of cells and occurrences for each age.
They are individually stored as one .png file for each age in a folder
within your working directory, and below is an example of how one would
look like for the Katian age.

Phyla that were filtered to only look at specific classes (such as only
bivalves and gastropods in the phylum Mollusca to remove pelagic classes
like ammonoids, or only trilobites in Arthropoda to remove terrestrial
insects) are in color, while phyla that were not filtered for specific
classes are in gray.
...
This is an example of one output for the Katian age. The rest are in the
interval_maps folder we generated in your wd.
...
How bad can the disparity in occurrences be, you ask? well, below I will
show you how each age can host wildly different amounts of occurrences.
To constrain for that, I will later show you my subsampling procedure
and why I prioritize examining homogenization as a function of distance.
...
occ_counts <- occ_counts |>
  dplyr::mutate(interval.ma = round(interval.ma, 4)) |>
  dplyr::left_join(
    interval.ma |> dplyr::mutate(interval.ma = round(interval.ma, 4)),
    by = "interval.ma"
  )
# bring in the interval names, then build a combined label
occ_counts <- occ_counts |>
  dplyr::mutate(
    label = paste0(early_interval, " (", interval.ma, ")"),
    label = factor(label, levels = label)  # locks in the n_occ sort order
  )

p <- ggplot(occ_counts, aes(x = n_occ, y = reorder(label, n_occ))) +
...
ggplot(occ_counts, aes(x = n_occ, y = reorder(early_interval, n_occ))) +
  geom_col(fill = "gray30", width = 0.8, color = "white", linewidth = 0.3) +
...
Here we perform subsampling without replacement on our stage-level
datasets using 999 iterations. For each age we randomly sample 15
occurrences per cell and repeated the process as stated above. The
results are subsampled datasets (cell-specific) saved as nested objects
within a larger list. These are subsequently, merged into single master
dataframes (i.e., the cells).

For each stage we create individual dataframes based on the cell units
and store these into separate lists. Then, we bootstrap these cells so
they each have 15 occurrences, 1000 times.

```{r subsample, warning=FALSE, cache=TRUE, cache.lazy=FALSE}
# split pbdb into a list of lists, each of which is one age:
pbdb_age <- split(pbdb, pbdb$interval.ma)

# subsample each cell for each age down to 15 occurrences 1000 times.
set.seed(31)

boot_pbdb <- purrr::map(1:1000, ~ {
purrr::map(pbdb_age, ~ {
.x |>
dplyr::group_by(cell) |>
...
# boot_pbdb is only ever used to build boot_combined and is not referenced
# again anywhere below, freeing it here roughly halves peak memory during
# this step regardless of how many replicates n_reps is
rm(boot_pbdb)
gc()
...
We begin by calculating the bootstrap statistics of Jaccard similarities
for each of the 1000 bootstrapped subsamples for each age, then
calculate the average Jaccard value per age.

We then run a second calculation of how much the Jaccard similarity has
changed in one age since its previous age. This is to ensure we are
examining the direct aftereffects of environmental or ecological
perturbations.

We finally get Jaccard as a function of distance for the 1000 samples
per age, then calculate it as change in Jaccard, and finally find the
95% confidence inerval of the similarity for each distance bin.

```{r jaccard, message=FALSE, warning=FALSE, cache=TRUE, cache.lazy=FALSE, dependson="subsample"}
...
```{r jaccard-distance, warning=FALSE, message=FALSE, cache=TRUE, cache.lazy=FALSE, dependson="subsample"}
...
replicate_means_distance2 <- replicate_means_distance |>
  arrange(desc(interval.ma), dist_bin) |>
  group_by(replicate, dist_bin) |>              
  mutate(diff = reps_jacc - lag(reps_jacc)) |>
  ungroup()
...
### Distance-bin significance and robust signal per age

For each time interval and distance bin: we look at the mean change in
Jaccard similarity, plus the 2.5th and 97.5th percentile of the
bootstrap distribution (a 95% CI). A bin is significant if that CI
excludes zero, i.e. the upper bound is below zero or the lower bound is
above zero.

Each age then gets a `robust_significant` flag: TRUE if at least 66% of
its distance bins are individually significant and all agree on
direction. There's no effect-size floor, coverage and direction are the
only conditions. Direction itself isn't encoded separately in the plots,
it's already visible from the sign of `mean_diff` on the y-axis (points
above zero vs below zero).

```{r dist-bin-sig, warning=FALSE, message=FALSE}

table_dist_bin_significance <- replicate_means_distance2 |>
  dplyr::group_by(interval.ma, dist_bin) |>
  dplyr::summarise(
    mean_diff  = mean(diff, na.rm = TRUE),
    lowci_diff = quantile(diff, 0.025, na.rm = TRUE),
  ...
  dplyr::mutate(significant = upci_diff < 0 | lowci_diff > 0)

table_dist_bin_significance <- table_dist_bin_significance |>
  dplyr::left_join(interval.ma, by = "interval.ma") |>
  dplyr::mutate(
    mass = dplyr::case_when(
      str_detect(early_interval, "(Induan|Hirnantian|Famennian|Danian|Rhuddanian|Hettangian)$") ~ "mass",
      TRUE ~ "background"
    )
  ) |>
  dplyr::arrange(interval.ma, dist_bin)

write.csv(table_dist_bin_significance, "table_S_distance_bin_significance.csv",
          row.names = FALSE)

table_dist_bin_significance |>
  gt::gt() |>
  gt::tab_header(title = "Table S#. Change in Jaccard similarity by time interval and distance bin") |>
  gt::fmt_number(columns = c(mean_diff, lowci_diff, upci_diff), decimals = 4) |>
  gt::cols_label(early_interval = "Age", interval.ma = "Age (Ma)", dist_bin = "Distance bin",
                 mean_diff = "Mean \u0394 Jaccard", lowci_diff = "2.5%", upci_diff = "97.5%",
                 significant = "Significant", mass = "Extinction type")

pct_threshold <- 0.66

age_majority_sig <- table_dist_bin_significance |>
  dplyr::group_by(interval.ma) |>
  dplyr::summarise(
    n_bins            = dplyr::n(),
    n_significant     = sum(significant, na.rm = TRUE),
    pct_significant   = n_significant / n_bins,
    n_signs_among_sig = dplyr::n_distinct(sign(mean_diff[significant])),
    sign_consistent   = n_signs_among_sig <= 1,
    median_effect_significant = dplyr::if_else(
      n_significant > 0, median(abs(mean_diff[significant]), na.rm = TRUE), NA_real_
    ),
    robust_significant = dplyr::coalesce(
      pct_significant >= pct_threshold & sign_consistent,
      FALSE
    ),
    .groups = "drop"
  ) |>
  dplyr::select(-n_signs_among_sig) |>
  dplyr::left_join(interval.ma, by = "interval.ma") |>
  dplyr::arrange(interval.ma)

write.csv(age_majority_sig, "table_S_age_robust_significance.csv", row.names = FALSE)

age_majority_sig |>
  gt::gt() |>
  gt::tab_header(title = "Table S#. Age-level robust significance summary") |>
  gt::fmt_number(columns = c(pct_significant, median_effect_significant), decimals = 3) |>
  gt::cols_label(early_interval = "Age", interval.ma = "Age (Ma)", n_bins = "Distance bins",
                 n_significant = "Bins significant", pct_significant = "% significant",
                 sign_consistent = "Same direction",
                 median_effect_significant = "Median |effect| (sig bins)",
                 robust_significant = "Robust signal")

diff_ci_dist <- diff_ci_dist |>
  dplyr::left_join(age_majority_sig |> dplyr::select(interval.ma, robust_significant),
                    by = "interval.ma")

jacc_final <- jacc_final |>
  dplyr::left_join(age_majority_sig |> dplyr::select(interval.ma, robust_significant),
                    by = "interval.ma")
```
...
# Ave. Jaccard plot:
# gray open circle = not robust, <66% of bins significant, or  both.
 a <- ggplot(jacc_final, 
       aes(x = interval.ma, y = mean_jacc, colour = robust_significant)) +
  scale_x_continuous(breaks = seq(0,550, by = 100)) +
  scale_x_reverse() +
  geom_point(aes(shape = robust_significant), size = 2.5) +              # one point per interval
  ylim(0, 0.25) +
  scale_color_manual(values = c(`FALSE` = "gray70", `TRUE` = "#cc0077"),
                      labels = c(`FALSE` = "Not a robust signal", `TRUE` = "\u226566% of bins significant, same direction"),
                      name = NULL) +
  scale_shape_manual(values = c(`FALSE` = 1, `TRUE` = 16), guide = "none") +
   geom_text(aes(label = ifelse(robust_significant, early_interval, "")), vjust = -1, hjust = 0.45, size = 4) +
  labs( x = "Geologic Age",
...
b <- ggplot(jacc_final, 
       aes(x = interval.ma, y = mean_diff, colour = robust_significant)) +
  scale_x_continuous(breaks = seq(0, 550, by = 100)) +
  scale_x_reverse() +
  geom_point(aes(shape = robust_significant), size = 2.5) +
  ylim(-0.2, 0.2) +
  ...
  scale_color_manual(values = c(`FALSE` = "gray70", `TRUE` = "#cc0077"),
                      ...
                      name = NULL) +
  scale_shape_manual(values = c(`FALSE` = 1, `TRUE` = 16), guide = "none") +
  geom_text(aes(label = ifelse(robust_significant, early_interval, "")),
...
c_labels <- diff_ci_dist |>
  dplyr::filter(robust_significant) |>
  dplyr::group_by(early_interval) |>
  dplyr::filter(dist_bin == max(dist_bin, na.rm = TRUE)) |>
  dplyr::ungroup()
...
c <- diff_ci_dist |> 
  ggplot(aes(x = dist_bin, 
             ...
             colour = robust_significant)) +     
  geom_line(linewidth = 1.3, position = position_dodge(width = 0.3), alpha = 0.8) + 
  geom_point(aes(shape = robust_significant), size = 1.8,
             position = position_dodge(width = 0.3)) +
  geom_text(data = c_labels, aes(label = early_interval),
            hjust = -0.1, vjust = 0, size = 4, show.legend = FALSE) +
  coord_cartesian(xlim = c(-0.05, 19000), ylim = c(-0.25, 0.25)) +
  geom_hline(yintercept = 0, linewidth = 0.5) + 
  scale_color_manual(values = c(`FALSE` = "gray70", `TRUE` = "#cc0077"),
                      labels = c(`FALSE` = "Not a robust signal", `TRUE` = "\u226566% of bins significant, same direction"),
                      name = NULL) +
...
::: callout-warning
The quantified Czekanowski's coeffificent is incredibly computationally
burdensome because it is an abundance-based measurement. As such, this
is used to check the veracity of any statistically meaningful changes
that occurred in the Phanerozoic (the Katian-Hirnantian-Rhuddanian; and
the Changhsingian-Induan- -Olenekian-Anisian) and not all 90-100
geologic ages.
:::
...
# 
# # We then split based on distinct cell pairs. This will creates a nested list with X splits each dataframe i.e. 999. We also remove any genera (i.e. accepted name) were 0 occurrences is recorded between cell pairs.
...
Count occurrences per genus in the Hirnantian to determine whether they
match the disaster taxa in literature, such as the genus Hirnantia.
...
Locate and remove all wastebin taxa, following the methods of *Plotnick
and Wagner (2005)*. First, using the full database, locate all wastebin
taxa throughout the "before" and "after" intervals by finding the 5 most
frequent genus occurrences.
...
Next, follow all the steps from **Visualization of Cells and
Occurrences** down to **Czekanowski indices** again. You will notice
slight changes to the number of cells since filtering out the wastebin
taxa reduces the number of occurrences we have.

# **Climate Stripes**

We'll begin with the generation of the Climate Stripes figure, since
it's fairly straightforward.

Please load in the dataset from the Supplementary Materials of *Scotese
(2021)*, which has the average global temperature per million years. In
that file, you should calculate the average temperature per geologic
age. If you can't do that, then no worries! The file
*Scotese_intervalT.csv* is made available in Github for you to use and
has all the information needed for this script.

The following block of code creates two plots: The first is a climate
stripe of Global Average Temperature (in degrees C) per geologic age,
and the second is a climate stripe of climate *change* per geologic age
(e.g., the Induan age became 5 degrees warmer than its previous age, the
Changhsingian):
...
Temps$Age <- factor(Temps$Age, levels = rev(levels(Temps$Age))) #levels = rev() will reverse the x axis
Temps$T_change <- as.numeric(Temps$T_change)
...
## Foote's calculation of per-capita extinction intensity

Net we calculate Foote's per-capita extinction intensity using the
simple equation from his 2000 publication. This was the gray portion of
Figure 1, which was overlaid on the climate stripe.

First, let's load in and clean the data. You can find the Pbdb .csv file
in our Github repository as well.

```{r foote}

pbdb_raw <- read.csv(file = '~/Documents/BioHom/pbdb.data.Nov2024.csv')

# radiometric dates
interval.ma <- pbdb_raw |>
  filter(!is.na(early_interval) & !is.na(min_ma)) |>
  group_by(early_interval) |>
  summarise(interval.ma = min(min_ma)) |>
  ungroup() |>
  distinct() |>
...
# filter taxa
pbdb <- pbdb_raw |>
  filter(phylum %in% c("Echinodermata", "Bryozoa") | 
         class %in% c("Gastropoda", "Bivalvia", "Trilobita", 
                      "Rhynchonellata", "Strophomenata", "Anthozoa"))

# coordinate cleaning
cl <- cc_val(pbdb, value = "flagged", lat = "paleolat", lon = "paleolng")
cl_rec <- pbdb[!cl, ]
pbdb <- pbdb |> cc_val(lat = "paleolat", lon = "paleolng")

# taxonomy cleaning
b_ranks <- c("phylum", "class", "order", "family", "accepted_name")
b_suff <- list(NULL, NULL, NULL, NULL, c("ina", "ella", "etta"))
pbdb2 <- check_taxonomy(pbdb, suff_set = b_suff, ranks = b_ranks,
                        verbose = FALSE, clean_name = TRUE,
                        resolve_duplicates = TRUE, jump = 5)
pbdb <- pbdb2$data[1:nrow(pbdb), ]
...
# head(fadlad)
# sum(fadlad$fad == fadlad$lad)  # single-interval taxa
# nrow(fadlad)
...
::: callout-note
### Extinction Intensity (Foote, 2000)

$$ext = \frac{N_b - N_{bt}}{N_b}$$
:::

...where $N_b$ = boundary-crossers (taxa present at the start of the
interval), $N_{bt}$ = taxa that survived the interval, and $\Delta t$ =
interval duration in Ma.

```{r agepairs, warning=FALSE}

# Create a matrix of age-pairs using interval.ma
 
agepairs <- interval.ma |>
  select(early_interval, interval.ma) |>
  distinct() |>
  arrange(desc(interval.ma)) |>  # oldest to youngest
  mutate(
    next_interval.ma    = lead(interval.ma),
   duration            = interval.ma - next_interval.ma  # interval length in Ma
  ) |>
  filter(!is.na(next_interval.ma))

# verify durations look reasonable
agepairs |> 
  filter(early_interval %in% c("Maastrichtian", "Changhsingian", "Hirnantian")) |>
  select(early_interval, interval.ma, next_interval.ma, duration)

```

### Get `Nb` and `Nbt`

$N_{b}$ will be taxa that originated at or before `interval.ma`, and
went extinct after or at `next_interval.ma`.

$N_{bt}$ will be taxa that originated at or before `interval.ma`, and
went extinct after `next_interval.ma`

```{r nb_nbt, warning =FALSE}
...
extinction_rate <- agepairs |> rowwise() |>
  ...
  mutate(
    Nb  = sum(fadlad$fad > interval.ma & fadlad$lad <= interval.ma),
    Nbt = sum(fadlad$fad > interval.ma & fadlad$lad < interval.ma),

    # per-capita rate per Ma (Foote 2000 full formulation)
    q = ifelse(Nb > 0 & Nbt > 0 & duration > 0,
               -log(Nbt / Nb) / duration,
               NA_real_),

    # proportional extinction 
    prop_ext = ifelse(Nb > 0, (Nb - Nbt) / Nb, NA_real_)
  ) |>
  ungroup()

# use proportional extinction instead bc of short duration q inflation

extinction_sub <- extinction_rate |>
  arrange(desc(interval.ma)) |>
  mutate(
    interval_ma_factor = factor(interval.ma, levels = unique(interval.ma)),
    prop_ext_norm = prop_ext / max(prop_ext, na.rm = TRUE)
  )

extinction_plot <- ggplot(extinction_sub, aes(x = interval_ma_factor, y = prop_ext)) +
  geom_col(fill = "gray50", width = 1) +
  theme_void() +
  labs(title = "Proportional Extinction") +
...
ggsave("extinction_plot.pdf", plot = extinction_plot, width = 12, height = 6, dpi = 300)
```

## Climate/Extinction/Jaccard plots put together\*\*

Now we match the Foote extinction intensity plot with the climate
stripes so we can overlay them.

```{r ext-plot, warning =FALSE, message=FALSE}

# reload Temps
Temps <- read.csv(file = '~/Documents/BioHom/Temperature-change/Scotese_intervalT.csv') |>
  select(-c(ScoteseAge, GAT, T_change_1)) |>
  na.omit() |>
  mutate(
    ave_T    = as.numeric(ave_T),
    T_change = as.numeric(T_change)
  )

# match each Temps age to nearest PBDB interval
nearest_match <- function(x, table) {
  table[which.min(abs(table - x))]
}

Temps$interval.ma <- sapply(Temps$Age, nearest_match,
                             table = interval.ma$interval.ma)

# join interval names
interval_lookup <- data.frame(
  early_interval = interval.ma$early_interval,
  interval.ma    = as.numeric(interval.ma$interval.ma)
)

Temps <- Temps |>
  left_join(interval_lookup, by = "interval.ma") |>
  rename(early_interval = early_interval.x) |>
  select(-early_interval.y)

# build x axis from full Temps using interval.ma numeric values
all_levels <- Temps |>
  arrange(desc(Age)) |>
  pull(interval.ma) |>
  unique() |>
...
Temps_all <- Temps |>
  arrange(desc(Age)) |>
  mutate(x_shared = factor(as.character(interval.ma), levels = all_levels))

# shift extinction bars one interval younger
extinction_sub <- extinction_sub |>
  mutate(x_shared = factor(as.character(next_interval.ma), levels = all_levels))

# label with names
interval_labels <- Temps |>
  arrange(desc(Age)) |>
  select(interval.ma, early_interval) |>
  distinct() |>
  deframe()  # creates named vector: names = interval.ma, values = early_interval

# add to plot
foote_climate <- ggplot() +
  geom_tile(data = Temps_all,
            aes(x = x_shared, y = 0.5, fill = T_change),
            height = 1) +
  scale_fill_gradient2(low = "#39A9C9", mid = "#F8F8FF", high = "red3",
                       midpoint = 0, name = "T change (°C)") +
  geom_col(data = extinction_sub,
           aes(x = x_shared, y = prop_ext_norm),
           fill = "gray20", width = 1, alpha = 0.4) +
  scale_x_discrete(limits = all_levels,
                   labels = interval_labels) +  # swap numeric for names
  theme_void() +
  ...
  labs(title = "")
foote_climate
ggsave("foote_climate.pdf", plot = foote_climate, width = 12, height = 6, dpi = 300)

```

And now, add the jaccard change plot on top:

```{r final, warning=FALSE, message=FALSE}

# jacc_final <- jacc_final |>
#   mutate(early_interval = coalesce(early_interval.x, early_interval.y)) |>
#   select(-early_interval.x, -early_interval.y)
...
# add x_shared to match ages
jacc_final <- jacc_final |>
  mutate(x_shared = factor(early_interval, levels = name_levels))

# verify
sum(is.na(jacc_final$x_shared))  # should be 2 (Stage 10 and Stage 3)
...
# shift extinction bars one interval younger using name lookup
next_lookup <- interval.ma |>
  arrange(desc(interval.ma)) |>
  mutate(next_early_interval = lead(early_interval)) |>
  select(early_interval, next_early_interval)

# build lookup from interval.ma to early_interval name
ma_to_name <- interval.ma |>
  select(early_interval, interval.ma) |>
  distinct()

# use next_interval.ma to find the next interval's name
extinction_sub <- extinction_sub |>
  left_join(ma_to_name, by = c("next_interval.ma" = "interval.ma"),
            suffix = c("", "_next")) |>
  mutate(x_shared = factor(early_interval_next, levels = name_levels))

#  rebuild foote_climate
foote_climate <- ggplot() +
  geom_tile(data = Temps_all,
            aes(x = x_shared, y = 0.5, fill = T_change),
            height = 1) +
  ...
  geom_col(data = extinction_sub,
           aes(x = x_shared, y = prop_ext_norm),
           fill = "gray20", width = 1, alpha = 0.4) +
  scale_x_discrete(limits = name_levels) +
  theme_void() +
  theme(legend.position = "bottom",
        plot.margin = margin(10, 10, 40, 10),
        aspect.ratio = 0.25,
...
#  rebuild jaccard change plot
# gray open circle = not robust: <66% of bins significant, disagree on
# direction, or median effect size among sig bins < 0.04. filled = passes all three.
jacc_change_plot <- ggplot(jacc_final,
                           aes(x = x_shared, y = mean_diff, colour = robust_significant)) +
  scale_x_discrete(limits = name_levels, drop = FALSE) +  # keep all levels
  geom_point(aes(shape = robust_significant), size = 2.5) +
  ylim(-0.2, 0.2) +
  geom_hline(yintercept = 0, linewidth = 0.5) +
  scale_color_manual(values = c(`FALSE` = "gray70", `TRUE` = "#cc0077"),
                      ...
                      name = NULL) +
  scale_shape_manual(values = c(`FALSE` = 1, `TRUE` = 16), guide = "none") +
  geom_text(aes(label = ifelse(robust_significant,
                               early_interval, "")),
            vjust = -1, hjust = 0.45, size = 4) +
  geom_errorbar(aes(ymin = lowci_diff, ymax = upci_diff),
                width = 0.5, alpha = 0.3) +
  labs(x = NULL,
       y = "Change in Jaccard Similarity") +
  theme_classic() +
...
# remove aspect.ratio from both plots and add explicit plot widths

jacc_change_plot <- jacc_change_plot +
  theme(aspect.ratio = NULL,
        plot.margin = margin(5, 5, 0, 5))

foote_climate <- foote_climate +
  theme(aspect.ratio = NULL,
        plot.margin = margin(0, 5, 5, 5))

# combine with fixed widths
combined <- (jacc_change_plot / foote_climate) +
  plot_layout(heights = c(2, 1)) &  # jacc taller than climate stripe
  theme(legend.position = "right")

combined
ggsave("jacc_foote_climate.pdf", plot = combined,
       width = 20, height = 8, dpi = 300)

```

## Bivariate Plot (Temperature Change + Extinction Rate)

Now we create a bivariate plot to visualize how temperature change and
extinciton rate plot for the ages.

```{r, warning = FALSE, message=FALSE}


# merge extinction and temperature data by interval
bivariate_data <- extinction_sub |>
  left_join(
    Temps |> select(interval.ma, early_interval, ave_T, T_change),
    by = c("next_interval.ma" = "interval.ma"),
    suffix = c("", "_next")  
  ) |>
  filter(!is.na(prop_ext) & !is.na(T_change)) |>
  mutate(abs_T_change = abs(T_change))

# the end of the series ends up with artificially high prop extinction because of the way the calculation is done, so remove them
bivariate_data<- bivariate_data|>
  filter(interval.ma >= 5) |>
  arrange(desc(interval.ma)) |>
  mutate(
    interval_ma_factor = factor(interval.ma, levels = unique(interval.ma)),
    prop_ext_norm      = prop_ext / max(prop_ext, na.rm = TRUE)
...
bivariate_plot <- ggplot(bivariate_data, aes(x = abs_T_change, y = prop_ext)) +
  geom_point(aes(size = Nb), colour = "gray30", alpha = 0.7) +
  geom_smooth(method = "lm", colour = "firebrick", se = TRUE) +
  geom_text_repel(aes(label = ifelse(prop_ext > 0.33, early_interval_next, "")),
                  size = 3,
                  box.padding = 0.5,       # space around label
                  point.padding = 0.3,     # space between label and point
                  segment.size = 0.2,      # thin connecting line
                  segment.alpha = 0.5,
                  max.overlaps = Inf) +
  scale_size_continuous(name = "Boundary crossers (Nb)",
                        breaks = c(500, 1000, 1500, 2000)) +
  labs(x = "Absolute Temperature Change (°C)",
       y = "Proportional Extinction",
       title = "") +
  theme_bw() +
  theme(
...
bivariate_plot
ggsave("bivariate_ext_temp.pdf", plot = bivariate_plot,
       width = 8, height = 6, dpi = 300)

# Color by jaccard change
jacc_by_age <- jacc_final |>
  group_by(early_interval) |>
  summarise(mean_diff = mean(mean_diff, na.rm = TRUE))

bivariate_data <- bivariate_data |>
  left_join(jacc_by_age, by = c("early_interval_next" = "early_interval"))

sum(is.na(bivariate_data$mean_diff))

bivariate_plot2 <- ggplot(bivariate_data, aes(x = abs_T_change, y = prop_ext)) +
  geom_point(aes(size = Nb, colour = mean_diff), alpha = 0.8) +
geom_smooth(method = "lm", colour = "firebrick", se = TRUE) +
geom_text_repel(aes(label = ifelse(prop_ext > 0.33, early_interval_next, "")),
                  size = 3,
                  box.padding = 0.5,
 ...
 scale_size_continuous(name = "Boundary crossers (Nb)",
                        breaks = c(500, 1000, 1500, 2000)) +
scale_colour_gradient2(low = "black", mid = "goldenrod4", high = "goldenrod1",
                        midpoint = 0.05, name = "Change in\nJaccard Similarity") +
  labs(x = "Absolute Temperature Change (°C)",
       y = "Proportional Extinction",
       title = "") +
  theme_bw() +
...
bivariate_plot2
ggsave("bivariate2", plot = bivariate_plot2, device = "pdf",
       width = 8, height = 6, dpi = 300)
```

# **Judd et al. 2024 Climate Stripes**

```{r, message=FALSE, warning=FALSE}
phanda <- read_csv("https://raw.githubusercontent.com/EJJudd/PhanDA/main/5_Outputs/PhanDA_GMSTandCO2_percentiles.csv")

JTemps <- phanda |>
  select(Stage, UpperAge, GMST_50) |>
  rename(early_interval = Stage, Age = UpperAge, ave_T = GMST_50) |>
  mutate(Age = as.numeric(Age)) |>
  arrange(desc(Age)) |>
  mutate(T_change = ave_T - lag(ave_T))

JTemps$ave_T <- as.numeric(JTemps$ave_T)
JTemps$Age   <- factor(JTemps$Age, levels = JTemps$Age)

Judd_phan_climate <- JTemps |>
  ggplot(aes(x = factor(Age), y = 1, fill = ave_T)) +
  geom_tile(height = 1) +
  geom_text(aes(label = Age), y = 0.5, size = 2.5, angle = 90, vjust = 1) +
  scale_fill_gradient2(low = "#39A9C9", mid = "#F8F8FF", high = "red3",
                       midpoint = 18, name = "Global Ave Temp (°C)") +
  labs(title = "Mean Global Temperature by Age (°C)") +
  theme_void() +
  theme(legend.position = "bottom", plot.margin = margin(20, 10, 40, 10))

print(Judd_phan_climate)

Temps$Age      <- factor(Temps$Age, levels = rev(levels(Temps$Age)))
Temps$T_change <- as.numeric(Temps$T_change)

Judd_climate_plot <- JTemps |>
  ggplot(aes(x = Age, y = 1, fill = T_change)) +
  geom_tile() +
  scale_fill_gradient2(low = "#39A9C9", mid = "#F8F8FF", high = "red3",
                       midpoint = 0, name = "Climate Change (°C)") +
  geom_text(aes(label = Age), y = 0.95, size = 2.5, angle = 90, vjust = 1.5) +
  labs(title = "T Change by Geologic Age") +
  theme_void() +
  theme(axis.text = element_blank(), axis.ticks = element_blank(),
        panel.grid = element_blank(), legend.position = "bottom",
        aspect.ratio = 0.25, plot.margin = margin(20, 10, 40, 10))
...
```{r judd, message=FALSE}
JTemps <- JTemps |>
  mutate(x_shared = factor(early_interval, levels = name_levels))

sum(is.na(JTemps$x_shared))

judd_climate <- ggplot() +
  geom_tile(data = JTemps, aes(x = x_shared, y = 0.5, fill = T_change), height = 1) +
  scale_fill_gradient2(low = "#39A9C9", mid = "#F8F8FF", high = "red3",
                       midpoint = 0, name = "T change (°C)") +
  geom_col(data = extinction_sub, aes(x = x_shared, y = prop_ext_norm),
           fill = "gray20", width = 1, alpha = 0.4) +
  scale_x_discrete(limits = name_levels, drop = FALSE) +
  theme_void() +
...
jacc_change_plot <- jacc_change_plot +
  theme(aspect.ratio = NULL, plot.margin = margin(5, 5, 0, 5))

judd_climate <- judd_climate + theme(aspect.ratio = NULL)

combined_judd <- (jacc_change_plot / judd_climate) +
  plot_layout(heights = c(2, 1), guides = "collect") &
  theme(legend.position = "bottom")

combined_judd
ggsave("jacc_judd_climate.pdf", plot = combined_judd, width = 16, height = 8, dpi = 300)
```

## Scotese vs. Judd: Temperature Change Correlation

```{r scotese-judd-comparison, message=FALSE, warning=FALSE}
Temps_c <- read.csv(file = '~/Documents/BioHom/Temperature-change/Scotese_intervalT.csv') |>
  select(-c(ScoteseAge, GAT, T_change_1)) |>
  na.omit() |>
  mutate(Age = as.numeric(Age), ave_T = as.numeric(ave_T), T_change = as.numeric(T_change)) |>
  select(early_interval, Age, ave_T, T_change) |>
  distinct() |>
  filter(Age >= 2.6, Age <= 467.3)
...
JTemps_c <- phanda |>
  ...
  arrange(desc(Age)) |>
  mutate(T_change = ave_T - lag(ave_T)) |>
  filter(Age >= 2.6, Age <= 470.0)

compare_df <- Temps_c |>
  left_join(JTemps_c |> select(early_interval, T_change) |> rename(T_change_judd = T_change),
            by = "early_interval") |>
  rename(T_change_scotese = T_change) |>
  mutate(match_type = ifelse(!is.na(T_change_judd), "name", "age"))

nearest_match <- function(x, table) table[which.min(abs(table - x))]

unmatched_idx <- which(is.na(compare_df$T_change_judd))
for (i in unmatched_idx) {
  nearest_age <- nearest_match(compare_df$Age[i], JTemps_c$Age)
  compare_df$T_change_judd[i] <- JTemps_c$T_change[JTemps_c$Age == nearest_age][1]
}

compare_df <- compare_df |>
  filter(!is.na(T_change_scotese), !is.na(T_change_judd)) |>
  mutate(sign_agree  = sign(T_change_scotese) == sign(T_change_judd),
         abs_scotese = abs(T_change_scotese),
         abs_mean    = (abs(T_change_scotese) + abs(T_change_judd)) / 2)

if (sum(is.finite(compare_df$T_change_scotese) & is.finite(compare_df$T_change_judd)) < 3) {
  stop("compare_df has fewer than 3 finite paired observations.")
}

pearson_all  <- cor.test(compare_df$T_change_scotese, compare_df$T_change_judd, method = "pearson")
spearman_all <- cor.test(compare_df$T_change_scotese, compare_df$T_change_judd,
                         method = "spearman", exact = FALSE)

cat("Overall (n =", nrow(compare_df), "):",
    "r =", round(pearson_all$estimate, 3),
    "R² =", round(pearson_all$estimate^2, 3),
    "p =", signif(pearson_all$p.value, 3), "\n")
cat("Spearman ρ =", round(spearman_all$estimate, 3),
    "p =", signif(spearman_all$p.value, 3), "\n")
cat("Sign agreement:", sum(compare_df$sign_agree), "/", nrow(compare_df), "\n")
```
