...
This Quarto report accompanies the manuscript *Coupling of severe climate change and extinction drive biogoegraphic homogenization in deep time* submitted to *Proceedings B*.
...
# Ave. Jaccard plot: 
 a <- ggplot(jacc_final, 
  ...
  ylim(0, 0.25) +
  scale_color_manual(values = c("gray60", "#db6600")) +
...
b <- ggplot(jacc_final, 
  ...
  scale_color_manual(values = c("grey60", "#db0096")) +
...
Net we calculate Foote's per-capita extinction intensity using the simple equation from his 2000 publication. This was the gray portion of Figure 1, which was overlaid on the climate stripe.

First, let's load in and clean the data. You can find the Pbdb .csv file in our Github repository as well.
...
...where $N_b$ = boundary-crossers (taxa present at the start of the interval), $N_{bt}$ = taxa that survived the interval, and $\Delta t$ = interval duration in Ma.
...
Now we match the Foote extinction intensity plot with the climate stripes so we can overlay them.
...
jacc_change_plot <- ggplot(jacc_final,
                           aes(x = x_shared, y = mean_diff, colour = mass)) +
  scale_x_discrete(limits = name_levels, drop = FALSE) +  # keep all levels
  geom_point(size = 2.5) +
  ylim(-0.2, 0.2) +
  geom_hline(yintercept = 0, linewidth = 0.5) +
  scale_color_manual(values = c("grey60", "#db0096")) +
  geom_text(aes(label = ifelse(mean_diff > 0.049 | mean_diff <= -0.049,
                               early_interval, "")),
            vjust = -1, hjust = 0.45, size = 4) +
...
Now we create a bivariate plot to visualize how temperature change and extinciton rate plot for the ages.
